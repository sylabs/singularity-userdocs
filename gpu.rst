...
-  ``nvidia-container-cli`` must be installed on your host, owned by the
   ``root`` user. Its path must be set in ``singularity.conf``. This
   value will be set at build time if ``nvidia-container-cli`` is found
   on the search ``$PATH``.
